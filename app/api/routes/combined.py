...
def process_video(
    ...
    try:
        ...
        # Update status to processing (2 hour TTL)
        ...
        # Update status with transcript included
        loop.run_until_complete(redis_service.set_status(
            request_id,
            {
                "status": "processing",
        ...
        # Store partial result with just transcript (1 hour TTL)
        partial_result = {
            "video_id": video_id,
        ...
        # Store complete result (24 hour TTL)
        complete_result = {
            "video_id": video_id,
            "transcript": transcript,
            "insights": insights,
        ...
        try:
            if 'loop' not in locals() or loop.is_closed():
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
